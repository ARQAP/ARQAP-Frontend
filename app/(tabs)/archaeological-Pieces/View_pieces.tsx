...
export default function ViewPieces() {
  ...
  const filtered = pieces.filter((p) => {
    ...
    if (
      ...
      return false; // placeholder category filter
    return true;
  });
        ...
        <View style={{ width: "90%", maxWidth: 700, padding: 16 }}>
          ...
          <View style={{ display: "flex", gap: 12 }}>
            {filtered.map((p) => (
              <TouchableOpacity
                key={p.id}
                onPress={() => router.push(`/(tabs)/archaeological-Pieces/View_piece?id=${p.id}`)}
                activeOpacity={0.9}
                style={{
                  backgroundColor: '#fff',
                  borderRadius: 8,
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                  <View style={{ flex: 1 }}>
                    <View style={{ flexDirection: 'row', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
                      <Badge text={p.shelf || ''} background={Colors.green} textColor={Colors.cremit} />
                      <Badge text={p.level || ''} background={Colors.brown} textColor={Colors.cremit} />
                      <Badge text={p.column || ''} background={Colors.black} textColor={Colors.cremit} />
                    </View>

                    <Text style={{ fontSize: 20, fontWeight: '700', marginBottom: 8, fontFamily: 'CrimsonText-Regular' }}>{p.name}</Text>
                  ...
                  <View style={{ marginLeft: 12, alignItems: 'center' }}>
                    <TouchableOpacity
                      onPress={(e) => {
                        // prevent parent touch from firing
                        e.stopPropagation?.();
                        router.push(`/(tabs)/archaeological-Pieces/Edit_piece?id=${p.id}`);
                      }}
                      style={{ padding: 8 }}
